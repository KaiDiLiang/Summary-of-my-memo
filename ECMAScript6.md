...
## **最常用的ES6特性**
  ...
  ### **`箭头函数` ：** arrow function    
  ### **`字符串` :** &ensp;\`\`（字符串模板可以换行写字符串）， 新增`.includes()`和`.repeat()`
...
```
   ...
   <p><strong>es5中var只有全局作用域和函数作用域，没有块状作用域的概念</strong></p>
...
```
   ...
   &ensp;<p><strong>es5不合理第一种是：</strong>内层变量覆盖外层变量。而 `let则实际上为JavaScript新增了块状作用域，用它声明的变量，只在let所在的代码块内有效`。</p>
...
```
  ...
  <p><strong>不合理第二种是：</strong>用来计数的循环变量泄露为全局变量。
...
```
   ...
   <p>上面的代码中，变量i是var声明的，在全局范围内都有效，所以每次循环，新的i值都会覆盖旧值，导致最后输出的是最后一轮的i的值。而使用let则不存在该问题。</p>
...
```
  ...
  <p>当尝试去改变用const声明的常量时，浏览器就会报错。</p>
...
---
 ...
 ### **`类` ：** class , **`继承` ：** extends , **`超类` ：** super（相当于父类构造函数的调用）
  <p>ES6 引入了Class（类）这个概念，新的class写法让对象原型的写法更清晰、更像面向对象编程的语法。</p>
...
```         
&ensp;<p>以上代码用 **`class`** 定义了一个“类”，可以看到有 `一个 `**constructor**` 方法（构造函数方法）`，而 ` this ` 关键字则代表 ` 实例对象 `。</p>

  ### **` constructor ` 内定义的`方法`和`属性`是`实例对象自己的`，而` constructor `外定义的`方法`和`属性`则是所有实例对象可以共享的。**

  &nbsp;<p>`class`之间可通过`extends`关键字实现`继承`,这比es5的通过修改原型链实现继承要清晰得多。
  
  上面代码定义了一个 `Cat类`，该类通过  `extends 关键字` ,继承了 `Animal 类` 的 `所有属性` 和 `方法`。</p>
  ...
  #### **`Es6的继承机制`：先创造`父类的实例对象this`（因而必须先调用 super方法 ）,再用 `子类的构造函数方法` 修改this。**
...
```
   ...
   #### **箭头函数体内的 `this对象`,指向 `定义时所在的对象`，而 `不是使用时所在的对象`。**
   #### **箭头函数没有自己的 `this`, `它的this是继承外部的`, 因此 `箭头函数内部的this就是外层代码块的this`。**
...
<p>使用es6的字符串模板``,不必关注换行转义符，直接换行即可，内部还支持表达式</p>
...
---
   ...
   ### **`解构` :** destructuring，可直接将数组或对象中的值直接分别导出到多个变量中
   #### es6允许按照一定模式，`从数组和对象中提取值，对变量进行赋值`，该模式称为`解构`。
